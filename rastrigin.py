...
def rastrigin(x,scale):
    ...
    x1=x[...,0]
    x2=x[...,1]
    return -(2*scale + np.sum(x**2- scale * np.cos(2*np.pi*x),axis=-1))
...
#this scale
scale=10
#we want the function to be the loglikelihood, so exp it to get the true target function
z=np.exp(rastrigin(gridstack,scale))
...
def loglike(theta):
    return float(rastrigin(theta,scale)), []
