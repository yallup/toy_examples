...
def make_animation_frames(grid,z,samples,filename,frames_dir="frames"):
    ...
    for i in np.arange(0, n_frames, 1):
        ...
        f, a = plt.subplots(nrows=1, ncols=2, sharey=True,sharex=True, gridspec_kw=gs, figsize=[8, 4])
        ...
        a[1].set_title("Nested Sampling live points")
        ...
        # f.tight_layout()
        f.savefig(os.path.join(anim_dir, str(i) + ".png"),dpi=150)

    
    proc=subprocess.Popen(["convert", "-delay", "50",*[os.path.join(anim_dir,str(x)+".png") for x in np.arange(n_frames-1)],"-delay","250",os.path.join(anim_dir,str(n_frames-1)+".png"),os.path.join(filename,filename)+".gif"])
    proc.communicate()
